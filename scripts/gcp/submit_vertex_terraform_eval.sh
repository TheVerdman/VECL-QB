...
if [[ -z "${HF_TOKEN:-}" ]]; then
  print -u2 "HF_TOKEN is required. Export it in your shell before launching the Terraform eval."
  exit 2
fi

PKG_DIR="/tmp/vecl-qb-terraform-eval-${JOB_TS}"
PACKAGE_TGZ="/tmp/vecl-qb-terraform-eval-${JOB_TS}.tar.gz"
CONFIG_YAML="/tmp/vecl-qb-terraform-eval-${JOB_TS}.yaml"
DISPLAY_NAME="vecl-qb-gemma-terraform-eval-${JOB_TS}"

mkdir -p "$PKG_DIR/terraform_eval_job"
cp -R vecl "$PKG_DIR/vecl"
cp scripts/terraform_eval_gemma.py "$PKG_DIR/terraform_eval_job/__main__.py"
touch "$PKG_DIR/terraform_eval_job/__init__.py"
cp tests/qb/fixtures/terraform_eval_v0.json "$PKG_DIR/terraform_eval_job/terraform_eval_v0.json"
...
setup(
    name="vecl-qb-terraform-eval",
    version="0.0.0",
    packages=find_packages(),
    py_modules=["sitecustomize"],
    package_data={"terraform_eval_job": ["terraform_eval_v0.json"]},
    install_requires=[
        "networkx>=3.3",
        "numpy>=1.25,<1.28",
        "pydantic>=2.7",
...
PACKAGE_URI="$BUCKET/packages/vecl-qb-terraform-eval-${JOB_TS}.tar.gz"
...
- machineSpec:
  ...
  pythonPackageSpec:
    ...
    packageUris:
    - ${PACKAGE_URI}
    pythonModule: terraform_eval_job
    env:
    - name: HF_TOKEN
      value: "${HF_TOKEN}"
