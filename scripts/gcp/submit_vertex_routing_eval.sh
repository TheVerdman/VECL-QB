...
if [[ -z "${HF_TOKEN:-}" ]]; then
  print -u2 "HF_TOKEN is required. Export it in your shell before launching the Vertex routing eval."
  exit 2
fi

PKG_DIR="/tmp/vecl-qb-routing-eval-${JOB_TS}"
PACKAGE_TGZ="/tmp/vecl-qb-routing-eval-${JOB_TS}.tar.gz"
CONFIG_YAML="/tmp/vecl-qb-routing-eval-${JOB_TS}.yaml"
DISPLAY_NAME="vecl-qb-gemma-routing-eval-${JOB_TS}"

mkdir -p "$PKG_DIR/routing_eval_job" "$PKG_DIR/scripts"
cp -R vecl "$PKG_DIR/vecl"
cp scripts/smoke_gemma.py "$PKG_DIR/scripts/smoke_gemma.py"
touch "$PKG_DIR/scripts/__init__.py"
cp scripts/routing_eval_gemma_stockfish.py "$PKG_DIR/routing_eval_job/__main__.py"
touch "$PKG_DIR/routing_eval_job/__init__.py"
cp tests/qb/fixtures/routing_eval_v0.json "$PKG_DIR/routing_eval_job/routing_eval_v0.json"
...
setup(
    name="vecl-qb-routing-eval",
    version="0.0.0",
    packages=find_packages(),
    py_modules=["sitecustomize"],
    package_data={"routing_eval_job": ["routing_eval_v0.json"]},
    install_requires=[
        "networkx>=3.3",
        "numpy>=1.25,<1.28",
        "pydantic>=2.7",
...
PACKAGE_URI="$BUCKET/packages/vecl-qb-routing-eval-${JOB_TS}.tar.gz"
