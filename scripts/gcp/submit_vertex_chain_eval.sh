...
VECL_ROUTING_MAX_NEW_TOKENS="${VECL_ROUTING_MAX_NEW_TOKENS:-96}"
VECL_CHAIN_EVAL_MAX_ENTRIES="${VECL_CHAIN_EVAL_MAX_ENTRIES:-30}"
STREAM_LOGS="${STREAM_LOGS:-true}"

if [[ -z "${HF_TOKEN:-}" ]]; then
  print -u2 "HF_TOKEN is required. Export it in your shell before launching the Vertex chain eval."
  exit 2
fi

PKG_DIR="/tmp/vecl-qb-chain-eval-${JOB_TS}"
PACKAGE_TGZ="/tmp/vecl-qb-chain-eval-${JOB_TS}.tar.gz"
CONFIG_YAML="/tmp/vecl-qb-chain-eval-${JOB_TS}.yaml"
DISPLAY_NAME="vecl-qb-gemma-chain-eval-${JOB_TS}"

mkdir -p "$PKG_DIR/chain_eval_job" "$PKG_DIR/scripts"
...
cp scripts/chain_eval_gemma_stockfish.py "$PKG_DIR/chain_eval_job/__main__.py"
touch "$PKG_DIR/scripts/__init__.py"
touch "$PKG_DIR/chain_eval_job/__init__.py"
cp tests/qb/fixtures/routing_eval_v0.json "$PKG_DIR/chain_eval_job/routing_eval_v0.json"
...
setup(
    name="vecl-qb-chain-eval",
    version="0.0.0",
    packages=find_packages(),
    py_modules=["sitecustomize"],
    package_data={"chain_eval_job": ["routing_eval_v0.json"]},
    install_requires=[
        "networkx>=3.3",
        "numpy>=1.25,<1.28",
        "pydantic>=2.7",
...
PACKAGE_URI="$BUCKET/packages/vecl-qb-chain-eval-${JOB_TS}.tar.gz"
...
- machineSpec:
  ...
  pythonPackageSpec:
    ...
    - name: VECL_CHAIN_EVAL_MAX_ENTRIES
      value: "${VECL_CHAIN_EVAL_MAX_ENTRIES}"
